...
namespace ProductApplication
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            order1.products.Add(new Product("book", 1, 12.49m, true, false));
            order1.products.Add(new Product("music CD", 1, 14.99m, false, false));
            order1.products.Add(new Product("chocolate bar", 1, 0.85m, true, false));
            WriteData(order1);

            //Order2
            Order order2 = new Order();
            order2.products.Add(new Product("imported box of chocolates", 1, 10m, true, true));
            order2.products.Add(new Product("imported bottle of perfume", 1, 47.5m, false, true));
            WriteData(order2);

            //Order3
            Order order3 = new Order();
            order3.products.Add(new Product("imported bottle of perfume", 1, 27.99m, false, true));
            order3.products.Add(new Product("bottle of perfume", 1, 18.99m, false, false));
            order3.products.Add(new Product("packet of headache pills", 1, 9.75m, true, false));
            order3.products.Add(new Product("box of imported chocolates", 1, 11.25m, true, true));
            WriteData(order3);

            Console.ReadKey();
        }
        public static void WriteData(Order order)
        {
            order.products.ForEach((item) =>
            {
                item.Compute();
                order.totalSelfPrice += item.selfPrice * item.num;
                order.totalSaleTax += item.salesTax * item.num;
                Console.WriteLine($"{item.num} {item.name}:{item.selfPrice}");
            });
            Console.WriteLine($"Sales Taxes:{order.totalSaleTax}");
            Console.WriteLine($"Total:{order.totalSelfPrice}");
            Console.WriteLine();
        }
    }
    /// <summary>
